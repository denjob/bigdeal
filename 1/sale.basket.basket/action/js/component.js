;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		lazyLoad: function()
		{
			...
			{
				/** addon for delete not exist items */
				var exists_data = this.result['BASKET_ITEM_RENDER_DATA'];
				var ar_exists_data = [];
				for(var t in exists_data) {
					ar_exists_data.push(exists_data[t].ID);
				}
				var shown_data = BX.findChild(this.getCacheNode(this.ids.itemListTable),{
					'tag':'tr',
					'class': 'basket-items-list-item-container',
					'attribute':{'data-entity':'basket-item'}
				},true,true);
				for (var i in shown_data) {
					var id = shown_data[i].id,
						h = shown_data[i].clientHeight;
					id = id.replace(this.ids.item,"");
					if(BX.util.in_array(id,ar_exists_data) === false) {
						this.deleteBasketItem(id, false, true);
						...
						if (BX.type.isDomNode(itemListContainer) && BX.type.isDomNode(itemList)) {
							itemListContainer.style.minHeight = (itemListContainer.clientHeight - h) + 'px';
							itemList.style.minHeight = (itemList.clientHeight - h) + 'px';
						}
					}	
				}
				/**--------------------------------------------------*/
